...
class face_rec():
    def run(self, client_json, face_recognition):
        ...
        client_json[2] =  client_json[2].split(',')
        child_pic_dir = []
        for item in client_json[2]:
            child_pic_dir.append(rootPath + item)
        face_names = []

        if not(os.path.isfile(image_path)):
            # 文件路径错误
            return '{"result":false,"msg":"\u6587\u4ef6\u8def\u5f84\u9519\u8bef"}'

        # 获取所有的图片文件
        alist = []
        for dir_path in child_pic_dir:
            if not(os.path.exists(dir_path)):
                # 文件夹路径错误
                return '{"result":false,"msg":"\u6587\u4ef6\u5939\u8def\u5f84\u9519\u8bef"}'
            allFile = os.walk(dir_path)
        ...
        retJson = {"result": True, "msg": face_names,
                   "coordinate": face_locations, "time": 'use time: %s S' % (end-start)}
